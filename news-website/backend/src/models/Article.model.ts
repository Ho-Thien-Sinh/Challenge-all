...
// Interface for Article attributes
...
// Interface for optional attributes when creating a new Article
...
@Table({
  ...ModelUtils.getCommonModelOptions('articles', 'Article'),
  paranoid: false,
})
export class Article extends Model<ArticleAttributes, ArticleCreationAttributes> implements ArticleAttributes {
  ...
  // Relationship with User will be defined in index.ts
  public author?: any;

  // Override getter/setter for images
  get images(): string[] {
    return ModelUtils.parseJsonArray(this.getDataValue('images'));
  }

  set images(value: unknown) {
    this.setDataValue('images', ModelUtils.stringifyJsonArray(value));
  }
}
...
// Article model initialization function
