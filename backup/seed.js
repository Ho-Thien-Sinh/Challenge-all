...
async function seedDatabase() {
  try {
    ...
    if (count === 0) {
      ...
      logger.info(`Added ${sampleArticles.length} sample articles to database`);
      console.log(`Added ${sampleArticles.length} sample articles to database`);
    } else {
      logger.info(`Database already has ${count} articles, no need to add sample data`);
      console.log(`Database already has ${count} articles, no need to add sample data`);
    }
  } catch (error) {
    logger.error(`Error adding sample data: ${error.message}`);
