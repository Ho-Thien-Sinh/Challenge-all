...
async function seedCrawlerData() {
  try {
    ...
    if (count > 0) {
      logger.info(`Already ${count} articles in database, no need to add sample data`);
      return;
    }
    ...
    for (const article of sampleArticles) {
      ...
      logger.info(`Added sample article: ${article.title}`);
    }
...
// Run the sample data seeding function
seedCrawlerData();